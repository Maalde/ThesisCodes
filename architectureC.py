...
def create_hidden(model, n_neurons):
    model.add(Conv2D(n_neurons, (3, 3), padding='same',strides=(2,2), kernel_initializer='he_uniform',activation='relu'))
    model.add(Conv2D(n_neurons, (3, 3),strides=(2,2), kernel_initializer='he_uniform',activation='relu'))
...
[create_hidden(model, i) for i in [64,128,512]]

model.add(Conv2D(1024, (9, 1),padding='same',strides=(1,1), kernel_initializer='he_uniform',activation='relu'))
...
#     mode="min",
#     baseline=None,
#     restore_best_weights=True),tf.keras.callbacks.CSVLogger('csv/architecture3_binary12_training_log.csv', separator=",", append=True)]
